...
else
{
	...
	else
	{
		...
		IniWrite,1,kodex.ini,Bundles,%BundleName%
		Bundles =
		Loop,bundles\*,2
		{
...
IfMsgBox, Yes
{
	IfNotExist %A_WorkingDir%\Kodex Export
		FileCreateDir,%A_WorkingDir%\Kodex Exports
	FileDelete,Kodex Exports\%CurrentBundle%.kodex
	FileAppend,%CurrentBundle%`n,Kodex Exports\%CurrentBundle%.kodex
	if (CurrentBundle = "Default")
		BundleDir = 
	else
	...
	Loop,%BundleDir%replacements\*,0
	{
		...
		FileAppend,%Hotstring%`n,Kodex Exports\%CurrentBundle%.kodex
		FileAppend,%replacement%`n,Kodex Exports\%CurrentBundle%.kodex
	}
	...
	FileAppend,�Triggers�`n,Kodex Exports\%CurrentBundkodexxter
	FileAppend,%EnterTrigs%`n,Kodex Exports\%CurrentBundle%.kodex
	FileAppend,%TabTrigs%`n,Kodex Exports\%CurrentBundle%.kodex
	FileAppend,%SpaceTrigs%`n,Kodex Exports\%CurrentBundle%.kodex
	FileAppend,%NoTrigs%`n,Kodex Exports\%CurrentBundle%.kodex
	MsgBox,4,Your bundle was successfully created!,Congratulations, your bundle was successfully exported!`nYou can now share your bundle with the world by sending them the %CurrentBundle%.kodex file.`nThey can add it to Kodex through the import feature. `n`nYour export can be found at %A_WorkingDir%\Kodex Export.`n`nWould you like to see the %CurrentBundle% bundle?
IfMsgBox, Yes
	Run,Kodex Exports\
}
...
IMPORT:
FileSelectFile, ImportBundle,,, Import Kodex bundle, *.kodex
if ErrorLevel = 0
{
...
IfMsgBox,Yes
{
	...
	IniWrite,0,kodex.ini,Bundles,%BundleName%
Bundles =
Loop,bundles\*,2
{
	Bundles = %Bundles%%A_LoopFileName%|
